        ...
        <ul class="navbar-nav ml-auto navbar-flex top-header-navbar">
            ...
            <li class="nav-item dropdown">
                <div class="">
                    ...
                    <div class="form-group mb-0 navbar-language-container">
                        <select name="user_lang" class="form-control select2" id="select-user-lang" style="width: 90px !important">
                            @foreach (app()->hrm_languages as $language)
                                <option {{ $language->language->code == userLocal() ? 'selected' : '' }}
                                value="{{ $language->country_code }}"
                                data-value="{{ $language->language->code }}">
                                    <span class="flag-icon flag-icon-us flag-icon-squared"></span> <span class="flag-text" style="text-transform: uppercase">{{ Str::of($language->language->code)->upper() }}</span> </option>
                            @endforeach
                        </select>
                    </div>
                </div>
                ...
                <div class="float-sm-right d-flex">
                    ...
                    @if (isAttendee()['checkin'] && !isAttendee()['checkout'])
                        ...
                        <span class="break_back_button">
                            ...
                            @else
                                <button data-toggle="modal" data-target="#take_break"
            ...
            <li class="nav-item dropdown md-device-hide notification_panel common-dropdown-btn mr-0"
                ...
                <a class="nav-link feather-bell {{ auth()->user()->unreadNotifications->count() > 0? 'moving-bell':'' }} " href="#">
                ...
                {{-- <a class="ml-2 mr-2 btn btn-warning box-shadow d-flex align-items-center sm-btn-with-radius">
                    <img class="zoom-in-zoom-out" src="{{ url('public/assets/bell.png') }}"
                        alt="" style=" width: 19px; height: 19px; padding:0px !important">
                </a> --}}

                {{-- notifications --}}
                <div class="notification-panel dropdownAnimation common-dropdown-content">
                    <h4 class="notification-panel-header">{{ _trans('common.Notifications') }}</h4>
                    <ul class="notifications">
                        @forelse (auth()->user()->unreadNotifications  as $key => $notification)
                            ...
                            @endphp
                            <a style="text-decoration: none;" href="{{$notification->data['actionURL']['web'] != null ? $notification->data['actionURL']['web'] : route('notification.details',$notification->id)}}">
                    ...
                    <footer class="notification-panel-footer">
                        <a href="{{ route('notification.index') }}" class="btn btn-primary text-capitalize">{{ _trans('common.View All') }}</a>
            ...
            <li class="nav-item dropdown profile_dropdown_panel common-dropdown-btn mr-0">
                ...
                <div
                    ...
                    <a class="dropdown-item" href="{{ route('notification.index') }}">
                        ...
                        {{ _trans('common.Notifications') }} <span class="text-red">({{ auth()->user()->unreadNotifications->count() }})</span>
                    </a>
...
<div class="modal fade" id="take_break" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel"
    aria-hidden="true">
    <div class="modal-dialog">
        <div class="modal-content data">
            <div class="modal-header text-center" style="background: #001f3f">
                <h5 class="modal-title text-white">{{ _trans('common.Take a break!') }}</h5>
...
<input type="text" hidden id="time_format" value="{{ settings('time_format')?? h }}">
...
@include('backend.partials.language_terms')
