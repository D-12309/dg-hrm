...
<aside class="main-sidebar elevation-4 dark-sidebar right-box-shadow">
        ...
        <a href="{{ route('admin.dashboard') }}" class="img-tag sidebar_logo">
            @if (env('FILESYSTEM_DRIVER') == 's3')
                <img src="{{ Storage::disk('s3')->url(config('settings.app')['company_logo_backend']) }}"
                    class="mt-3 logo-cus" alt="{{ config('app.name') }}" width='200'>
            @elseif(env('FILESYSTEM_DRIVER') == 'local')
                @php
                    if (config('settings.app')['company_logo_backend']) {
                        $URL = Storage::url(config('settings.app')['company_logo_backend111']);
                    } else {
                        $URL = url('public/assets/logo-white.png');
                    }
                @endphp
                <img src=" {{ @$URL }}" class="mt-3 logo-cus" alt="{{ config('app.name') }}" width='200'>
            @else
    ...
    <div class="sidebar mt-4">
        <nav class="padding_bottom">
            ...
            {{-- @dd(hasPermission('payroll_item_menu')) --}}
                ...
                @if (hasPermission('payroll_menu'))
                    <li class="nav-item has-submenu {{ set_menu([route('hrm.payroll_items.index')]) }}">
                        ...
                        <ul class="submenu collapse {{ set_active(['hrm/payroll*']) }}">
                            ...
                            @endif
                            @if (hasPermission('list_payroll_setup'))
                            ...
                            @if (hasPermission('advance_type_list'))
                                <li class="nav-item {{ menu_active_by_route(['hrm.payroll_advance_type.index']) }}">
                                    <a href="{{ route('hrm.payroll_advance_type.index') }}" class="nav-link">
                        ...
                        <ul
                            ...
                            @if (hasPermission('notice_menu'))
                                <li class="nav-item  {{ menu_active_by_route(['notice.index', 'notice.create', 'notice.edit']) }} ">
                                    <a href="{{ route('notice.index') }}"   class="nav-link  {{ menu_active_by_route(['notice.index', 'notice.create', 'notice.edit']) }} ">
                                       <span>{{ _trans('common.Notice') }}</span> 
                                     </a>
                                </li>
                            @endif
                ...
                @endif
                {{-- @dd(hasPermission('location_read')) --}}
                ...
                <li class="nav-item has-submenu">
                    ...
                    <ul class="submenu collapse  {{ set_active(['admin/company-setup*', 'hrm/weekend/setup*', 'hrm/holiday/setup*', 'hrm/duty/schedule*', 'hrm/shift*']) }}">                    
                        {{-- @if (hasPermission('company_settings_read')) --}}
                        ...
                        @if (hasPermission('company_setup_location'))
                            <li class="nav-item {{ menu_active_by_route(['company.settings.location', 'company.settings.locationCreate']) }}">
