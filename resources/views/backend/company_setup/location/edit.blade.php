                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group" id="_st">
                                        <label for="status_id">{{ _trans('common.Status') }} <span
                                                class="text-danger">*</span></label>
                                        <select id="status_id" class="form-control" required>
                                            <option value="" disabled>{{ _trans('common.Choose One') }}</option>
                                            <option {{ @$data['location']->status_id == 1 ? 'selected' : '' }}
                                                value="1" selected>{{ _trans('common.Active') }}</option>
                                            <option {{ @$data['location']->status_id == 4 ? 'selected' : '' }}
                                                value="4">{{ _trans('common.In-active') }}</option>
                                        </select>
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        <label for="is_office">{{ _trans('common.Is Office') }} <span
                                                class="text-danger">*</span></label>
                                        <select id="is_office" name="is_office" class="form-control" required>
                                            <option value="33" {{ @$data['location']->is_office == 33 ? 'selected' : '' }}>
                                                {{ _trans('common.Yes') }}</option>
                                            <option value="22" {{ @$data['location']->is_office == 22 ? 'selected' : '' }}>
                                ...
                                {{-- @if (checkFeature('user_wise_location_binds'))
                                    <div class="col-md-6">
                                        <div class="form-group" id="_st">
                                            <label for="employee_id">{{ _trans('common.Employee') }} <span
                                                    class="text-danger">*</span></label>
                                            <select id="employee_id" class="form-control" required>
                                                <option value="all" selected>{{ _trans('common.All') }}</option>
                                                @foreach ($data['users'] as $user)
                                                    <option {{ @$data['location']->employee_id == $user->id ? 'selected' : '' }} value="{{ $user->id }}">{{ $user->name }}</option>
                                                @endforeach
                                            </select>
                                        </div>
                                    </div>
                                @else
                                    <input type="hidden" id="employee_id" value="all">
                                @endif --}}
                                    <input type="hidden" id="employee_id" value="all">
                                <div class="col-lg-12">
                                    <div class="form-group">
                                        <label for="name">{{ _trans('settings.Location') }} <span
                                            ...
                                            <span class="btn btn-primary" id="detectLocation"
                                                data-toggle="locationPickerStore">
                                            ...
                                            <div class="col-md-6 d-none" id="location_input">
                                                <input id="pac-input" class="form-control controls location" type="text"
                                                    placeholder="Enter a location" required name="location"
...
@section('script')
    ...
    <script src="{{ asset('public/backend/js/__location_find_edit.js') }}"></script>
@endsection
