...
// ebrahim 
function showDetailsActionBtn($string, $param)
{
    return
        '<a href="javascript:;" data-toggle="modal" data-target="#package_details_show" class="dropdown-item" onclick="' . $param . '">
            ' . $string . '
        </a>';
}
...
if (!function_exists('roleToUserPermission')) {

    function roleToUserPermission()
    {
       $role_permissions=auth()->user()->role->permissions;

        $permission_array=[];
        foreach ($role_permissions as $key => $permissions) {
        ...
        return getType($permissions)=='array' ? $permission_array : $role_permissions;
    }
}
if (!function_exists('hasPermission')) {
    ...
    function hasPermission($key_word)
    {
        ...
        if (config('app.APP_BRANCH')!='saas' && auth()->user()->is_admin == 1) {
            return true;
        }elseif (in_array($key_word, $system_permissions)) {
        ...
        if ($attendance) {
            if ($attendance->check_out) {
                ...
                return [
                    ...
                    'out_time' => $attendance->check_out,
                    'stay_time' => $attendance->stay_time,
                ];
            }
        } else {
            return [
...
if (!function_exists('LandlordCompany')) {
    function LandlordCompany()
    {
        try {
            $data['company']=Company::with('configs','settings')->where('id', 1)
            ->first();
            //Company Configs
            $config_array     = [];
            foreach ($data['company']->configs as $key => $config) {
                $config_array[$config->key] = $config->value;
            }
            //Company Settings
            $setting_array     = [];
            foreach ($data['company']->settings as $key => $setting) {
                $setting_array[$setting->name] = $setting->value;
            }
            // $data['settings']   = $setting_array;
            $data['settings']   = array_merge($setting_array, $config_array);
            return $data;
        } catch (\Throwable $th) {
            return [];
        }    
...
if (!function_exists('LandlordSettings')) {
    function LandlordSettings($setting)
    {
        try {
            $data=LandlordCompany()['settings'][$setting];
           return $data;
        } catch (\Throwable $th) {
            return null;
        }    
