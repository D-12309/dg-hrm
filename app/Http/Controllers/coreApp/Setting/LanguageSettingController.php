...
{
    private $languate;
    private $hrm_language;
    public function __construct(LanguageRepository $language, HrmLanguageRepository $hrm_language)
    {
        $this->language = $language;
        $this->hrm_language = $hrm_language;
    }
    ...
    public function language()
    {
        ...
        $data['hrm_languages']=$this->hrm_language->pluckData($pluck_columns)->toArray();
        return view('backend.language.index',compact('data'));
    }

    public function dataTable()
    {
        return $this->hrm_language->dataTable();
    }
    ...
    public function setup($language){
        ...
        $data['hrm_language']=$this->hrm_language->getHrmLanguageInfo($language);
// return $data;
    ...
    public function store(Request $request)
    {
       ...
       try {
         $this->hrm_language->store($request);
            Toastr::success('Operation Successfully done','Success');
            return back();
       } catch (\Throwable $th) {
        Toastr::error('Something went wrong','Error');
    ...
    public function makeDefault($language_id){
      try {
           $result= $this->hrm_language->makeDefault($language_id);
           if ($result) {
                Toastr::success('Operation Successfully done','Success');
                return back();
           } else {
    ...
    public function deleteLang ($language_id){
      try {
           $result= $this->hrm_language->deleteLang ($language_id);
           if ($result) {
                Toastr::success('Operation Successfully done','Success');
                return back();
           } else {
    ...
    {
        try {
            $result = $this->hrm_language->setCountry($request);
            return response()->json([
                'success' => true,
