...
class CompanyController extends Controller
{
    ...
    {
        // $daterange      =   $request->all()['daterange'];
        // $from_date      =   trim(explode("-",$daterange)[0]);
        // $to_date        =   trim(explode("-",$daterange)[1]);
        // $request['from']=   Carbon::parse($from_date)->toDateTimeString();
        // $request['to']  =   Carbon::parse($to_date)->format('Y-m-d') .' 23:59:59';
    ...
    public function store(CompanyRequest $request)
    {
        ...
        try {
            ...
            if($data){
                ...
                Toastr::error(_trans('response.Company doesn\'t create! try again'), 'Error');
                return redirect()->back();
            }
