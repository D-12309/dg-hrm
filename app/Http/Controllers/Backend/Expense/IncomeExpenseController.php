...
class IncomeExpenseController extends Controller
{
    ...
    {
        try {
            $data['title'] = _trans('finance.Income/Expense List');
    ...
    {
        try {
            $data['title'] = __translate('Create Income/Expense');
    ...
    public function store(IncomeExpenseRequest $request)
    {
        try{
            ...
            if ($data) {
                Toastr::success(_translate('Income-Expense created successfully.'), 'Success');
        ...
        {
            try {
                $data['title'] = _trans('finance.Create Income/Expense');
                $data['suppliers'] = $this->incomeExpense->suppliers();
        ...
        public function update(IncomeExpenseRequest $request, $id)
        {
            try{
                ...
                if ($data) {
                    Toastr::success(_trans('response.Category has been updated successfully.'), 'Success');
                    return redirect()->route('income_expense.index');
                } else {
                    Toastr::error(_trans('response.Something went wrong!'), 'Error');
                    return redirect()->back();
        ...
        public function destroy($id)
        {
            try {
                ...
                if ($data) {
                    Toastr::success(_trans('finance.Income-Expense has been deleted successfully.'), 'Success');
                    return redirect()->route('income_expense.index');
                } else {
                    Toastr::error(_trans('response.Something went wrong!'), 'Error');
                    return redirect()->back();
