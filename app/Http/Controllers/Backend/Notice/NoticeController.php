...
class NoticeController extends Controller
{
    ...
    {
        $data['show'] = $this->notice->showWeb($id);
    ...
    public function pushNotificationHistory()
    {
        $data['title'] = _trans('notice.Push Notification History');
        $data['url'] = route('notice.historyDataTable');
        return view('backend.notice.history', compact('data'));
    }
    public function pushNotificationDetails($id)
    {
        $data['title'] = _trans('notice.Push Notification Details');
        $data['show'] = $this->notice->pushNotificationDetails($id);
        return view('backend.notice.push_notification_details', compact('data'));
    }
    public function historyDataTable()
    {
        return $this->notice->historyDataTable();
    }
    public function sendPushNotification(Request $request)
    {
        ...
        try {
            ...
            $push_notification->notification_type=$request->notification_type;
            $push_notification->notify_by=auth()->user()->id;
            ...
            if ($request->notification_type==1) {
                foreach ($request->users as $key => $user) {
                    ...
                    $user_notification=new PushNotificationUser();
                    $user_notification->push_notification_id=$push_notification->id;
