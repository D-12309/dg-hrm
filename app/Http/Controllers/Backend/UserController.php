...
class UserController extends Controller
{
        ...
        try {
            if (settings('login_method')=='pin') {
                if(strlen($request->pin) != 5){
                    Toastr::warning(_trans('response.Pin Length must be 5 digit'));
                    return back();
                }
            }
    ...
    public function store(UserStoreRequest $request)
    {
        ...
        try {
            ...
            //$user = User::create(['company_id'=>'1','name' => $request->name,'phone' => $request->phone,'email' => $request->email,'role_id' => $request->role_id,'password'=>$request->password , 'joining_date'=>$request->joining_date, 'department_id'=> $request->department_id, 'designation_id'=>$request->designation_id,'gender'=>$request->gender,'basic_salary'=>$request->basic_salary]);
            DB::commit();
    ...
    public function profileView(Request $request, User $user, $slug)
    {
      if (!myCompanyData($user->company_id)) {
        ...
        $data['title'] = _trans('common.Profile Details');
        $data['slug'] = $slug;
        $data['id'] = $user->id;
    ...
    public function edit(User $user)
    {
        try {
            ...
            if (settings('user_wise_permission') == null || settings('user_wise_permission')) {
                ...
                return view('backend.user.only_edit_view', compact('data'));
            }
