    ...
    public function update(ShiftReqeust $request, Shift $shift): \Illuminate\Http\RedirectResponse
    {
        ...
        try {
            if ($this->isExistsWhenUpdate($shift, $this->model, 'name', $request->name)) {
                ...
                Toastr::error("{$request->name} "._trans('response.Already exists'), _trans('response.Warning'));
                return redirect()->back();
            }
        } catch (\Exception $e) {
            Toastr::error(_trans('response.Something went wrong!'), _trans('response.Error'));
            return redirect()->back();
        }
    }
