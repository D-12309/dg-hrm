...
class AdvanceController extends Controller
{
    ...
    function store(Request $request)
    {
        ...
        try {
            ...
            if ($result->original['result']) {
                // dd(route('hrm.payroll_advance_salary.index'));
    ...
    function update(Request $request, $id, $company)
    {
        $request->validate([
            'user_id' => 'required | max:191',
            'month' => 'required',
    ...
    function delete($id)
    {
        try {
            ...
            if (!$advance) {
                Toastr::error(_trans('response.Advance not found!'), 'Error');
                return redirect()->route('hrm.payroll_advance_salary.index');
            }
    ...
    {
        if (!$request->category) {
            Toastr::error(_trans('response.Please select category!'), 'Error');
            return redirect()->back();
        }
        if (!$request->account) {
            Toastr::error(_trans('response.Please select account!'), 'Error');
            return redirect()->back();
        }
        try {
            $params                = [
    ...
    public function advanceLoan($id)
    {
        try {
            ...
            if ($user) {
                ...
                Toastr::error(_trans('response.User Not Found.'), 'Error');
                return back();
            }
