    ...
    function table($request, $id)
    {
        ...
        return [
            'data' => $discussions->map(function ($data) {
                $action_button = '';
                if (hasPermission('task_discussion_view')) {
                ...
                if (hasPermission('task_discussion_update')) {
                    $action_button .= '<a href="' . route('task.edit', [$data->task_id, 'discussions', 'discussion_id=' . $data->id]) . '" class="dropdown-item"> ' . _trans('common.Edit') . '</a>';
                }
                if (hasPermission('task_discussion_delete')) {
