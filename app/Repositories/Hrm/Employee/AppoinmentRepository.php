...
class AppoinmentRepository extends BaseRepository
{
    ...
    public function getDetails()
    {
       ...
       if ($appoinment) {
            $appointmentDetails=[
                ...
                'other_participant' => $appoinment->partnerInfo(),
                // 'other_participant' => [
    ...
    public function store($request)
    {
        ...
        $notify_body = _trans('response.You have scheduled an appointment with').' '.auth()->user()->name.' - '.$this->dateFormatInPlainText($appoinment->date.' '.$appoinment->appoinment_start_at);

        $details = [
            'title' => _trans('response.New Appointment Scheduled'),
            'body' => $notify_body,
            'actionText' => 'View',
            'actionURL' => [
        ...
        if ($appoinment->appoinment_with != null) {
            $title=_trans('response.Appointment Notification');
            $this->sendCustomFirebaseNotification($appoinment->appoinment_with, 'appointment_request', '', 'appointment_request', $title, $notify_body);
            sendDatabaseNotification($appoinment->appoinmentWith, $details);
        }

        return $this->responseWithSuccess(_trans('response.Appoinment Created Successfully'), [], 200);
    }
    ...
    public function appoinmentChangeStatus($request)
    {
        ...
        if ($validator->fails()) {
            return $this->responseWithError(_trans('response.Validation field required'), $validator->errors(), 422);
        }
        $participant=AppoinmentParticipant::where('appoinment_id', $request->appoinment_id)->where('participant_id',auth()->user()->id)->first();
            if(!$participant){
                return $this->responseWithError(_trans('response.You are not participant of this appoinment'), [], 422);
            }
        ...
        $participant->save();
        return $this->responseWithSuccess(_trans('response.Appoinment Status Changed Successfully'), [], 200);
    }
