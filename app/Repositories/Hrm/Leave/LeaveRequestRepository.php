...
class LeaveRequestRepository
{
    ...
    public function store($request)
    {
        ...
        try {
            ...
            //     return $this->responseWithError('Permission denied', [], 400);
            // }
                ...
                if ($leaveType) {
                    ...
                    if ($leaveType->department_id == $userDepartment->department_id) {
                        ...
                        if ($user) {
                            ...
                            $details = [
                                'title' => _trans('response.New Leave Request'),
                                'body' => $notify_body,
                                'actionText' => _trans('common.View'),
                                'actionURL' => [
                                    'app' => 'leave_request',
                            ...
                            $title=_trans('response.Leave Request');
                            if ($leaveRequest->user->manager_id != null) {
                              $this->sendCustomFirebaseNotification($leaveRequest->user->manager_id, 'leave_approved', '', 'leave_approved', $title, $notify_body);
                                sendDatabaseNotification($leaveRequest->user->manager, $details);
                            }
                            if ($leaveRequest->user->myHr() != null) {
                                $this->sendCustomFirebaseNotification($leaveRequest->user->myHr()->id, 'leave_approved', '', 'leave_approved', $title, $notify_body);
    ...
    public function leaveDataTable($leaveRequest)
    {
        return datatables()->of($leaveRequest->latest()->get())
            ...
            ->addColumn('available_days', function ($data) {
                $leaveSummary = resolve(LeaveService::class)->leaveSummary($data->user->id,'web');
                if ($leaveSummary->original['result']) {
    ...
    public function approveOrRejectOrCancel($id, $status): bool
    {
        ...
        if ($leaveRequest) {
            ...
            $leaveRequest->save();
            $title=_trans('response.Leave Notification');
            $notify_body=_trans('response.Leave Request').' '.$leaveRequest->status->name ;
            if ($status == 1) {
                //send notification to manager
                $notify_body=_trans('response.Leave Request Approved') ;
                if ($leaveRequest->user->manager_id != null) {
                    $this->sendCustomFirebaseNotification($leaveRequest->user->manager_id, 'leave_approved', '', 'leave_approved', $title, $notify_body);
                }
                //send notification to staff
                $this->sendCustomFirebaseNotification($leaveRequest->user->id, 'leave_approved', '', 'leave_approved', $title, $notify_body);
                $this->approvedBy($leaveRequest);
            } elseif ($status == 6) {
                //send notification to staff
                $this->sendCustomFirebaseNotification($leaveRequest->user->id, 'leave_rejected', '', 'leave_rejected', $title, $notify_body);
                $this->rejectedBy($leaveRequest);
            } elseif ($status == 7) {
                //send notification to staff
                $this->sendCustomFirebaseNotification($leaveRequest->user->id, 'leave_cancelled', '', 'leave_cancelled', $title, $notify_body);
                $this->cancelledBy($leaveRequest);
            } elseif ($status == 17) {
                //send notification to HR
                $this->sendCustomFirebaseNotification($leaveRequest->user->id, 'leave_referred', '', 'leave_referred', $title, $notify_body);
