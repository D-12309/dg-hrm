...
class VisitRepository
{
    ...
    public function __construct(Visit $visit ,VisitService $visitService)
    {
        $this->visit = $visit;
    ...
    public function dataTable($visits)
    {
        return datatables()->of($visits)
            ...
            ->rawColumns(array('employee_name', 'date', 'title', 'description', 'cancel_note', 'status', 'action'))
            ->make(true);
    }
    ...
    {
        $visit = $this->visit->with('visitImages')->where('id', $id)->first();
        return $visit;
    }
    public function visitScheduleArray($id)
    {
        $schedule_array=[];
        $visit = $this->visit->where('id', $id)->first();
        foreach ($visit->schedules as $key=> $schedule) {
            $schedule_array[$key] = [
                'id' => $schedule->id,
                'latitude' => $schedule->latitude,
                'longitude' => $schedule->longitude,
                'start_location' => $this->visitService->geolocationaddress($schedule->latitude,$schedule->longitude,),
                ];
        }
