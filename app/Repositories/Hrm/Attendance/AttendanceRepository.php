...
class AttendanceRepository
{
    ...
    public function locationCheck($request)
    {
        ...
        $user=User::where('id', $request->user_id)->with('userLocation')->first();
        if (checkFeature('user_wise_location_binds') && !$user->is_free_location) {
            $user_location=$user->userLocation;
            $distance = distanceCalculate($request->latitude, $request->longitude, $user_location->latitude, $user_location->longitude);
                if ($distance <= $user_location->distance) {
                    $locationInfo = true;
                }
    ...
    public function userAttendance($user, $request)
    {
        ...
        if ($user) {
            ...
            if (settings('location_check') && !$this->locationCheck($request) && auth()->user()->is_free_location==0) {
            ...
            if (count($attendance_status) > 0) {
                ...
                if ($attendance_status[0] == AttendanceStatus::LATE && $request['check_in_location'] != 'Device' && settings('multi_checkin') == 0) {
                    $validator = Validator::make($request->all(), [
                        'reason' => 'required',
                    ]);
