    ...
    function generateDatatable($content)
    {
        return datatables()->of($content)
            ->addColumn('action', function ($data) {
                ...
                // if (hasPermission('advance_salaries_invoice')) {
                //     $action_button .= '<a href="' . route('hrm.payroll_advance_salary.invoice', $data->id) . '" class="dropdown-item"> ' . _trans('common.Invoice') . '</a>';
                // }

                if (hasPermission('advance_salaries_delete') && $data->status_id != 5 && @$data->pay == 9) {
                    $action_button .= actionButton('Delete', '__globalDelete(' . $data->id . ',`hrm/payroll/advance-salary/delete/`)', 'delete');
                }
