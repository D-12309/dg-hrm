...
class PayrollSetUpRepository
{
    ...
    function delete_salary_setup($request,$id)
    {
        try {
            $commission_details=SalarySetupDetails::find($id)->delete();
            return $this->responseWithSuccess(_trans('message.Item Deleted successfully.'), $commission_details);
        } catch (\Throwable $th) {
            return $this->responseExceptionError($th->getMessage(), [], 400);
        }
    }
