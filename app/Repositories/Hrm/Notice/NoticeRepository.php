...
class NoticeRepository
{
    ...
    public function datatable($notice)
    {
        ...
        ->addColumn('action', function ($data) {
            $action_button = '';
            $action_button .= '<a href="' . route('notice.show', $data->id) . '" class="dropdown-item"> View</a>';
    ...
    public function historyDataTable()
    {
        ...
        return datatables()->of($push_notifications)
        ...
        ->addColumn('notify_by', function ($data) {
            return @$data->notifyBy->name;
        })
        ->addColumn('message', function ($data) {
            return Str::limit(@$data->message, 50);
        })
        ->addColumn('type', function ($data) {
            return $data->notification_type==1 ? _trans('common.Specific User') : _trans('common.All');
        })
        ->addColumn('action', function ($data) {
            $action_button = '';
            $action_button .= '<a href="' . route('notice.pushNotificationDetails', $data->id) . '" class="dropdown-item"> View</a>';
            $button = '<div class="flex-nowrap">
                <div class="dropdown">
                    <button class="btn btn-white dropdown-toggle align-text-top action-dot-btn" data-boundary="viewport" data-toggle="dropdown">
                        <i class="fas fa-ellipsis-v"></i>
        ...
        ->rawColumns(array('title', 'date', 'message', 'type', 'notify_by', 'action'))
        ->make(true);
    }
    function pushNotificationDetails($id){
        try {
            $notification=PushNotification::with('users','notifyBy','users.user')->find($id);
            return $notification;
        } catch (\Throwable $th) {
            return false;
        }
