...
//    }
    ...
    {
        $data = $this->appScreen->orderBy('position', 'asc')->get();
        return $data;
    }
    ...
    function AppScreenSortable($request)
    {
        try {
            foreach ($request->result as $key => $value) {
                $row_data=explode('=',$value);
                $appScreen = $this->appScreen->findOrFail($row_data[0]);
                $appScreen->position = $row_data[1];
                $appScreen->save();
            }
            return $this->responseWithSuccess(_trans('response.App Screen Menu Reordered'), [], 200);
            //json response
