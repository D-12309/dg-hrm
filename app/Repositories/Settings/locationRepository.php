...
class locationRepository
{
    ...
    function getAll()
    {
        $locations = $this->model->query()->where('company_id', auth()->user()->company_id)->get();
        return $locations;
    }
    ...
    function create($request)
    {
        try {
            if (@$request->user_id ) {
                $employee_id =  @$request->user_id == 'all' ? null : @$request->user_id;
            } else {
                $employee_id = null;
            }
            $where = [
            ...
            $location_bind->employee_id = $employee_id;
    ...
    function update($request, $id)
    {
        try {
            ...
            $location_check = $this->model->where($where)->where('id', '!=', $id)->first();
            if ($location_check) {
                return $this->responseWithError(_trans('Location already exists'), 'name', 422);
            }
