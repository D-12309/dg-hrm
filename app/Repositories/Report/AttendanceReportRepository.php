    ...
    public function attendanceDatatable($request)
    {
        ...
        return datatables()->of($attendance->latest()->get())
            ...
            ->addColumn('name', function ($data) {
                return $status = '<a href="' . route('employeeAttendance', $data->user_id) . '" target="_blank">' . @$data->user->name . '</a>';
            })
            ...
            ->addColumn('checkin', function ($data) {
                if ($data->in_status === 'OT') {
                    ...
                    $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $data->check_in_location . '"> <i class="fas fa-map-marker"></i> </span>';
                    $status .= '</div>';
                    return $status;
            ...
            ->addColumn('checkout', function ($data){
                ...
                if (@$data->check_out) {
                    if ($data->out_status === 'LT') {
                        ...
                        $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $data->check_out_location . '"> <i class="fas fa-map-marker"></i> </span>';
                        $status .= '</div>';
                        return $status;
...
//                    $checkInStatus = $this->checkInStatus($attendance->user_id, $attendance->check_in);
                    if ($attendance->in_status == AttendanceStatus::ON_TIME) {
                        ...
                        $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_in_location . '"> <i class="fas fa-map-marker"></i> </span>';
                        return $status;
                ...
                if ($attendance) {
                    if ($attendance->check_out) {
                        if ($attendance->out_status == AttendanceStatus::LEFT_TIMELY) {
                            ...
                            $status .= '<span class="badge badge-primary ml-2" data-toggle="tooltip" data-placement="top" title="' . $attendance->check_out_location . '"> <i class="fas fa-map-marker"></i> </span>';
                            return $status;

                        } elseif
                        ($attendance->out_status == AttendanceStatus::LEFT_EARLY) {
                            $status = '';
                        ...
                        ($attendance->out_status == AttendanceStatus::LEFT_LATER) {
